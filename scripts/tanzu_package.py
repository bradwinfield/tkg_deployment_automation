...
if not helper.check_for_result(["tanzu", "package", "repository", "list", "-A"], expression):
    ...
    rc = helper.run_a_command("tanzu package repository add " + tanzu_standard_package_repo_name + " -n " + package_namespace + " --url " + tanzu_package_registry + ":" + tanzu_package_registry_version)
    if rc != 0:
